...
class PDF extends FPDF
{
  ...
  //include "utilGeneral.php";
   ...
   function fnImprimeTitulo()
   {
      ...
      $pdf->Cell(60, 8 ,"NUMEROS GENERADORES",1,0,'C',1);
      $pdf->setY($PosYContrato);
      $pdf->Cell(60) ;
      $pdf->SetFont('Arial','B',8);          	   
      $pdf->MultiCell($tmpLimiteHoja - 60, 4 ,$arTitulos[1][8],1,'R',0);

      $pdf->setY($pdf->getY() + 1);
	  $PosYContrato=$pdf->getY();
	  if(!isset($xContrato))$xContrato=0.1;

	  $pdf->MultiCell( $tmpLimiteHoja - 90 ,13.5 ,$arTitulos[1][2],1,'L',0);
	  ...
	  if($rowCaratula = mysql_fetch_array($QryCaratulaGenerador))
 	  {
		  ...
		  $pdf->Cell(50,3,$rowCaratula['dFechaInicioEst']." AL ".$rowCaratula['dFechaFinalEst'],1,1,'L',0);
	   	  $pdf->Cell($xGenerador);
	      $pdf->SetFont('Arial','B',6);          	   
		  $pdf->Cell(10,3,"ANEXO",1,0,'L',1);
	      $pdf->SetFont('Arial','',6);          	   
		  $pdf->Cell(40,3,$sDescripcion,1,0,'L',0);
	      $pdf->SetFont('Arial','B',6);          	   
		  $pdf->Cell(25,3,"TIPO DE MONEDA",1,0,'L',1);
	      $pdf->SetFont('Arial','',6);          	   
		  $pdf->Cell(15,3,$rowCaratula['sMoneda'],1,1,'L',0);
	   	  $pdf->Cell($xGenerador);
	      $pdf->SetFont('Arial','B',6);          	   
		  ...
		  $pdf->MultiCell(50,6,"",1,'L',0);
	   	  $pdf->Cell($xGenerador);
	      $pdf->SetFont('Arial','B',6);          	   
		  $pdf->Cell(40,3,"PLANO PEMEX",1,0,'L',1);
	      $pdf->SetFont('Arial','',6);    
	  ...
	  $anchoCantidad = 15 ;
   	  ...
   	  $pdf->setY($yFilaCabecera);	  
	  $pdf->Cell($xContrato);
	  $pdf->MultiCell($anchoIsometrico,3,"PLANO REF. DE LA CONTRATISTA",1,'C',1);
   	  $pdf->setY($yFilaCabecera);	  
   ...
   function fnImprimePartidas()
   {
      ...
      $anchoCaracter = 0 ;
  	  ...
  	  $sqlPartidasGenerador = "Select e.iConsecutivo, e.sFaseObra, a.sNumeroActividad, a.sMedida, a.dCantidadAnexo, a.mDescripcion, e1.sIsometrico, e1.sPrefijo, e1.sIsometricoReferencia, e1.dCantidad, e1.iOrdenCambio from estimaciones e INNER JOIN estimacionxpartida e1 ON (e.sContrato = e1.sContrato And e.sNumeroOrden = e1.sNumeroOrden And e.sNumeroGenerador = e1.sNumeroGenerador and e1.lEstima = 'Si') INNER JOIN estimacionperiodo e2 on (e.sContrato =  e2.sContrato And e.iNumeroEstimacion = e2.iNumeroEstimacion) INNER JOIN actividadesxanexo a ON (e1.sContrato = a.sContrato and e1.sNumeroActividad = a.sNumeroActividad and a.sIdConvenio = '$sIdConvenio') where e.sContrato = '$sContrato' And e.sNumeroOrden = '$sNumeroOrden' And e.sNumeroGenerador = '$sNumeroGenerador' Order By e1.sIsometricoReferencia, a.iItemOrden, e1.sIsometrico, e1.sPrefijo" ;      
      $qryPartidas = mysql_query($sqlPartidasGenerador,$link);
      while($rowPartidas = mysql_fetch_array($qryPartidas))
      {
	   	  If ($sIsometricoReferencia != $rowPartidas['sIsometricoReferencia']) 
   	      {
				...
				$pdf->setY($yLineaOriginal - 11) ;
	 	   	    $pdf->Cell(145);
	  		    $pdf->Cell(50,3,$rowPartidas['sIsometricoReferencia'],0,0,'L',0);

		        $pdf->SetFont('Arial','',6);    

		    	$sIsometricoReferencia = $rowPartidas['sIsometricoReferencia'] ;
			    $sIsometricos = '' ;
				$sNumeroActividad = '' ;
		  }   	       
   	      If ($sNumeroActividad != $rowPartidas['sNumeroActividad'])   
   	      {
	          ...
	          If (($yLinea + (strlen(trim($rowPartidas['mDescripcion'])) * $anchoCaracter)) > $CuerpoPartida) 
	          {
			      ...
			      $pdf->SetFont('Arial','',5);    
  				  $pdf->setY($yLineaOriginal - 17) ;
	 		   	  $pdf->Cell(145);
	  			  $pdf->MultiCell(50,5,$rowPartidas['sFaseObra'],0,'L',0);
		   ...
		   If ($rowPartidas['sPrefijo'] != '') 
		    	If ($sIsometricos != '') 
	    	    	$sIsometricos = $sIsometricos . ', ' . $rowPartidas['sIsometrico'] . '-' . $rowPartidas['sPrefijo'] ;
    	    	Else
	        		$sIsometricos = $rowPartidas['sIsometrico'] . '-' . $rowPartidas['sPrefijo'] ;
	       Else
		  		If ($sIsometricos != '') 
		    		$sIsometricos = $sIsometricos . ', ' . $rowPartidas['sIsometrico'] ;
		    	Else
	    			$sIsometricos = $rowPartidas['sIsometrico'] ; 
...
//$pdf->Output('Numeros Generadores.PDF' , 'D'); 
