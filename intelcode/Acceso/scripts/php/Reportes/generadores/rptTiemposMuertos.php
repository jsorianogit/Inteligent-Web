...
if(isset($_POST['ordenSeleccionada'])){
   ...
   if (isset($_POST['ordenSeleccionada'])) $ordenSeleccionada =$_POST['ordenSeleccionada'];
}
else if ( isset($_REQUEST['ordenSeleccionada'])){
   if ( isset($_REQUEST['fechaIni']))  $dFechaInicio=formatoFechaPer($_REQUEST['fechaIni'],"Y-m-d");
   if ( isset($_REQUEST['fechaTerm'])) $dFechaFinal=formatoFechaPer($_REQUEST['fechaTerm'],"Y-m-d");
   if ( isset($_REQUEST['ordenSeleccionada']))$ordenSeleccionada =$_REQUEST['ordenSeleccionada'];
...
class PDF extends FPDF
{
   ...
   function fnLeeConfiguracion($sContrato)
   {
      ...
      $sqlTitulo = "SELECT
         ...
         FROM  contratos c2 INNER JOIN configuracion c ON (
            c.sContrato = c2.sContrato
         )
         Where c2.sContrato = '$sContrato'";
   ...
   function fnImprimeTitulo()
   {
     ...
     global $ordenSeleccionada;

      // busco el convenio activo cuando se imprimio el generador ...
     $sqlReporteDiario = "Select sIdConvenio from reportediario where sContrato = '$sContrato' And sNumeroOrden = '$ordenSeleccionada' And dIdFecha = '$dFechaFinal'" ;
     ...
     if($rowConvenio = mysql_fetch_array($QryConvenio))
     {
        ...
        $pdf->Cell(40,3,"PERIODO DE IMPRESION",1,0,'L',1);
         $pdf->SetFont('Arial','',6);
        $pdf->Cell(50,3,formatoFecha($dFechaInicio) . ' AL ' . formatoFecha($dFechaFinal),1,1,'L',0);
           $pdf->Cell($xGenerador);
         $pdf->SetFont('Arial','B',6);
        ...
        $pdf->Cell(40,3,"VIGENCIA DEL CONTRATO",1,0,'L',1);
         $pdf->SetFont('Arial','',6);
        $pdf->Cell(50,3,formatoFecha($rowConvenio['dFechaInicio']) ." AL ".formatoFecha($rowConvenio['dFechaFinal']),1,1,'L',0);
     }
...
//   $pdf->SetFillColor(237,243,216);
     ...
     $pdf->MultiCell($anchoHora + 4.5 ,2,"T. INAC. X CONTRATO",1,'C',1);
       ...
       fnFirmasAutorizadas ( $sContrato, $ordenSeleccionada, $dFechaFinal, 172, 'L') ;
   ...
   function fnImprimePartidas()
   {
     ...
     global $sfnResultHoras ,$ordenSeleccionada;
     ...
     $CuerpoPartida = 168 ;
      ...
      $Incremento = 0 ;
      $sTiempoTotal = '00:00' ;
      if($ordenSeleccionada=='Todas las Ordenes'){
      $sqlReportesDiarios = "Select
            sNumeroOrden,
            sNumeroReporte,
            dIdFecha,
            sIdTurno,
         ...
         where
            ...
            And sTiempoMuertoReal <> '00:00'
         Order By dIdFecha, sNumeroOrden" ;
      }
      else {
    $sqlReportesDiarios = "Select
            sNumeroOrden,
         ...
         where
            ...
            And lStatus = 'Autorizado'
            And sTiempoMuertoReal <> '00:00'
            and sNumeroOrden='$ordenSeleccionada'
         Order By dIdFecha, sNumeroOrden" ;
      }
   ...
   //      echo ($yLinea + $anchoCaracter) ;
        ...
        $sTiempoTotal = $sfnResultHoras ;
           ...
           $pdf->setY($yLinea);    
        $pdf->Cell($xColumna);
        $pdf->Cell($anchoFecha,4,formatoFecha($rowReportes['dIdFecha']),0,0,'L',0);
           $pdf->setY($yLinea);    
...
//      $pdf->SetFillColor(237,243,216);
        ...
        $sqlTiempos = "Select
                      ...
                      INNER JOIN tiposdemovimiento tm on (
                        t.sContrato = tm.sContrato
                        And t.sIdTipoMovimiento = tm.sIdTipoMovimiento
                      )
                      Where
                        t.sContrato = '$sContrato'
                        And t.sNumeroOrden = '$rowReportes[0]'
                        And t.dIdFecha = '$rowReportes[2]'
                        And t.sIdTurno = '$rowReportes[3]'
