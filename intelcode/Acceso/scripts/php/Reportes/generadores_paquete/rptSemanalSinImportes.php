...
class PDF extends FPDF
{
   ...
   function fnImprimeTitulo()
   {
      ...
      $pdf->MultiCell(70, 4 ,"RESUMEN SEMANAL DE NÚMEROS GENERADORES SIN IMPORTES",1,'L',1);
      $pdf->setY($PosYContrato);
      $pdf->Cell(70) ;
      $pdf->SetFont('Arial','B',8);          	   
      $pdf->MultiCell($tmpLimiteHoja - 70, 4 ,$arTitulos[1][8],1,'R',0);

      $pdf->setY($pdf->getY() + 1);
	  $PosYContrato=$pdf->getY();
	  if(!isset($xContrato))$xContrato=0.1;
	  $pdf->MultiCell( $tmpLimiteHoja - 90 ,8,$arTitulos[1][2],1,'J',0);
   ...
   function fnImprimePartidas()
   {
	  ...
	  $lImpreso = 'No' ;     
  	  ...
  	  $sqlPartidasGenerador = "Select e.iConsecutivo, e.sFaseObra, a.sNumeroActividad, a.sMedida, a.dCantidadAnexo, a.mDescripcion, a.dVentaMN,  sum(e1.dCantidad) as dCantidad, e.mComentarios from estimaciones e INNER JOIN estimacionxpartida e1 ON (e.sContrato = e1.sContrato And e.sNumeroOrden = e1.sNumeroOrden And e.sNumeroGenerador = e1.sNumeroGenerador and e1.lEstima = 'Si') INNER JOIN estimacionperiodo e2 on (e.sContrato =  e2.sContrato And e.iNumeroEstimacion = e2.iNumeroEstimacion) INNER JOIN actividadesxanexo a ON (e1.sContrato = a.sContrato and e1.sNumeroActividad = a.sNumeroActividad and a.sIdConvenio = '$sIdConvenio') where e.sContrato = '$sContrato' And e.sNumeroOrden = '$sNumeroOrden' And e.sNumeroGenerador = '$sNumeroGenerador' Group By e1.sNumeroActividad Order By a.iItemOrden" ;      
      ...
      $pdf->SetFont('Arial','BU',6);          	   
   	  $pdf->setY($yLinea);	  
	  $pdf->Cell($xColumna);
	  $pdf->MultiCell($tmpLimiteHoja,7,$mComentarios,0,'L',0);
...
//$pdf->Output('SemanalSinImportes.pdf','D'); 
$pdf->Output(); 
