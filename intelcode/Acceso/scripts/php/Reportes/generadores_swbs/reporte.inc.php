...
function setPrefijo($pref){
 $this->prefijo = $pref ;
}

function getPrefijo(){
   if($this->prefijo == "")
      return "Reporte_Correspondiente_a_";
   else
      return $this->prefijo;
   ...
   function PrintImage()
   {
      ...
      while($NumImg<$Tamanio)
      {
         ...
         if(file_exists("photo/".$nameFile[$NumImg][1]))
            $this->image("photo/".$nameFile[$NumImg][1],$x+550,$y,$Ancho,$Alto);
         $this->setY($y);
    ...
    if(!$this->headerset[$this->page]) {
       ...
       $this->MultiCell(305, 9,"INFORME COMPARATIVO DE CANTIDAD ANEXO CONTRA INSTALADO AL DIA $Fecha",1,1,1);
        ...
        if(strpos($orden ,"CONTRATO")!==false){
            $Tex = "DETALLES DE MOVIMIENTOS";
       }
      else{
            $Tex = "DETALLES DE MOVIMIENTOS DE LA ORDEN $orden";
         }
        $width = strlen($Tex) + 180;
        $this->MultiCell($width, 9,$Tex,1,1,1);
...
function mysql_report($query,$dump=false,$attr=array()){
    ...
    $this->Output($this->getPrefijo()."$sContrato.pdf");
    location($this->getPrefijo()."$sContrato.pdf");
}
