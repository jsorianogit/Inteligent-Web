...
class generadoresPDF extends PDF
{
   var $configExiste;
   function header(){}
   ...
   function fnImprimeTitulo()
   {
     ...
     $y = 11 ; 
      ...
      $pdf->MultiCell(($tmpLimiteHoja - ($tmpAnchoImagen + $tmpAnchoImagen + 40)),3,$arTitulos[1][4],0,'C');
           //la siguiente linea fue a peticon de rbtec...
      if ( $arTitulos[1][5] == "RBTEC S.A. DE C.V.")
      {
        $pdf->MultiCell(($tmpLimiteHoja),4,"UNIDAD DE INGENIERIA DE TELECOMUNICACIONES ZONA MARINA ",0,'C');
      }
      $pdf->SetFont('Arial','',5);              
     ...
     $pdf->Cell(20,6,"OBRA",0,0,'L',0);
          $pdf->SetFont('Arial','',7);                
     $pdf->MultiCell( $tmpLimiteHoja - 110 ,3,$arTitulos[1][2],0,1,'J',0);
   ...
   function fnImprimePartidas()
   {
     ...
     $CuerpoPartida = 160 ;
           $anchoCaracter = 0 ;
          $height = 4.5 ;
     $sqlPartidasGenerador = "Select e.iConsecutivo, a.sNumeroActividad, a.sMedida, a.dCantidadAnexo, a.mDescripcion, e1.sIsometrico, e1.sPrefijo, e1.dCantidad, e1.iOrdenCambio from estimaciones e 
...
'Actividad') 
where 
e.sContrato = '$sContrato' And e.sNumeroOrden = '$sNumeroOrden' And e.sNumeroGenerador = '$sNumeroGenerador' Order By a.iItemOrden, e1.sIsometrico, e1.sPrefijo" ;      
   ...
   {  
     ...
     $yFilaFirmas = 160 ;
      ...
      if($lStatus  == "Pendiente"){
         ...
         $this->TextWithRotation(20,190,'Sin Validacion',30,-30);
         $this->SetTextColor(0,0,0);
      }
      if($lStatus != "Autorizado"){
         $this->SetFont('Arial','',14);
         $this->SetTextColor(130,130,130);
         $this->TextWithRotation(125,190,'Sin Autorizacion',30,-30);
          $this->SetTextColor(0,0,0);
      }
...
//$pdf->Output();
#asignar nombre
$name = "reports/CaratuladeGenerador_$sContrato";
