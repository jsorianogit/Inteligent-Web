...
class numeroGeneradorPDF extends PDF
{
   var $configExiste;
  ...
  //include "utilGeneral.php";
   ...
   function fnImprimePartidas()
   {
     ...
     $sqlPartidasGenerador = "Select e.iConsecutivo, e.sFaseObra, a.sNumeroActividad, a.sMedida, a.dCantidadAnexo, a.mDescripcion, e1.sIsometrico, e1.sPrefijo, e1.sIsometricoReferencia, e1.dCantidad, e1.iOrdenCambio from estimaciones e 
...
'Actividad') 
where e.sContrato = '$sContrato' And e.sNumeroOrden = '$sNumeroOrden' And e.sNumeroGenerador = '$sNumeroGenerador' Order By e1.sIsometricoReferencia, a.iItemOrden, e1.sIsometrico, e1.sPrefijo" ;      
      ...
      while($rowPartidas = mysql_fetch_array($qryPartidas))
      {
        If ($sIsometricoReferencia != $rowPartidas['sIsometricoReferencia']) 
            ...
            If ($sIsometricoReferencia != "")
            {
               If ($sIsometricos != "")
               {
                  ...
                  $dCantidad = 0 ;
                                          ...
                                          $pdf->setY($yLineaAnterior);
                                          If ($yIncremento > $yLineaDescripcion)
                                             $pdf->Cell($tmpLimiteHoja, $yIncremento ,"",1,1,'C') ;               
                                          else
                                              $pdf->Cell($tmpLimiteHoja, $yLineaDescripcion , "",1,1,'C') ;
           ...
           $pdf->MultiCell($anchoDescripcion,3,trim($rowPartidas['mDescripcion']),0,'J',0);
                   $yLineaDescripcion = $pdf->getY() - $yLineaAnterior ;  
                   If ($anchoCaracter == 0 )
...
#asignar nombre
$name = "reports/Numeros_Generadores_$sContrato";
