    ...
    require("_avancesGlobales.php");
      ...
      {   //obtenemos el nombre de la base de datos....
	    ...
	    if ($BaseDatos <> 'rbtec')         
          {
            ...
            $Linea = $Linea + $height ;
                ...
                //if($TodosAutorizado==0){$dAvanceRealOrdenAnterior=$dAvanceRealOrdenActual=$dAvanceRealOrdenAcum=0;}
        ...
        $pdf->SetFont('Arial','B',6) ;                     
            ...
            $Linea = $Linea + $height ;
                ...
                if($TodosAutorizado==0){
                        ...
                        $dAvanceRealObraAnterior = "O  B  S  E";
                        $dAvanceRealObraActual="R    V    A    C";
                        $dAvanceRealObraAcum= "I  O  N  E  S";
                        $align1 = 'R';
                        $align2 = 'C';
                        $align3 = 'L';
                        $fondo = 1;
        ...
        $pdf->SetFont('Arial','',6) ;              
                $pdf->setY($Linea);       
                $pdf->Cell($xContrato + $posContratoX + $anchoTAvance);
                $pdf->Cell($anchoAvance ,$height ,$dAvanceRealObraAnterior ,1,0,$align1,$fondo);

                $pdf->setY($Linea);       
                $pdf->Cell($xContrato + $posContratoX + $anchoTAvance + $anchoAvance);
                $pdf->Cell($anchoAvance ,$height ,$dAvanceRealObraActual ,1,0,$align2,$fondo);

                $pdf->setY($Linea);       
                $pdf->Cell($xContrato + $posContratoX + $anchoTAvance + $anchoAvance + $anchoAvance);
                $pdf->Cell($anchoAvance ,$height ,$dAvanceRealObraAcum ,1,0,$align3,$fondo);
