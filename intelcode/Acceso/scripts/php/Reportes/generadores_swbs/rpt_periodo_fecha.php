...
function Periodos_Fechas($sContrato,$dFecha,$sNumeroOrden,$sIdTurno,$sIdConvenio)
{
   ...
   #informacion del contrato
    $sql="SELECT rd.sOperacionInicio,rd.sOperacionFinal,rd.sTiempoEfectivo,rd.sTiempoMuerto,rd.sTiempoMuertoReal,con.dFechaInicio, con.dFechaFinal,ot.sDescripcionCorta
         ...
         INNER JOIN convenios con ON (rd.sContrato=con.sContrato AND rd.sNumeroOrden='$sNumeroOrden' AND rd.sIdConvenio=con.sIdConvenio) 
   ...
   if($row=mysql_fetch_array($result))
   {
      ...
      if($rwTurno = mysql_fetch_array($rsTipoTurno)){
			$turnoTierra = $rwTurno["sOrigenTierra"]; 
	  }   	
      ...
      if($turnoTierra=="Si") {
	  	$des = "OBRA EN TIERRA";}
      else if($turnoTierra=="No") {
		$des = "OBRA EN PLATAFORMA";
      }else{
	  	$des = $row['sDescripcionCorta'];
      }
      
      $pdf->Cell(70,3,$des,1,1,'C',1);
      $pdf->SetFillColor(215,217,200);
         $pdf->Cell($xPeriodo);
      $pdf->Cell(40,3,"PERIODO DE CONTRATO",1,0,'L',1);
      $pdf->Cell(30,3,formatoFecha($row['dFechaInicio'])." AL ".formatoFecha($row['dFechaFinal']),1,1,'L',0);
         $pdf->Cell($xPeriodo);
   ...
   if($row=mysql_fetch_array($result))
   {
      ...
      $pdf->Cell(40,3,formatoFecha($row['dIdFecha']),1,1,'L',0);
         $pdf->Cell($xFecha);
