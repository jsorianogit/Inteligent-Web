...
/*###########################################################################################
   ...
   function fnImprimeTitulo()
   {
      ...
      $pdf->RoundedRect(10.1, $yFilaCabecera, $anchoDescripcionAct + $anchoDescripcAv, $height * 2 , 3.50, '1111');
   ...
   function fnImprimePartidas($sContrato,$dIdFecha,$sNumeroOrden,$sIdTurno,$sIdConvenio,$lStatus="")
   {
      ...
      //global $arTitulos, $link, $sContrato, $dIdFecha, $sNumeroOrden, $sIdTurno, $sIdConvenio, $lStatus;
      global $link,$pdf;
      //$lStatus = 'Autorizado' ;
   // echo "<br><br><br>contrato ".$sContrato."<br> fecha: ".$dIdFecha." <br>orden: ".$sNumeroOrden." <br>turno: ".$sIdTurno." <br>convenio: ".$sIdConvenio." <br>status: ".$lStatus."<br><br>";
	  if(!isset($xContrato))$xContrato=0.1;
	  $pdf->Cell($xContrato);
			...
			If ($rowGerencial = mysql_fetch_array($qryGerencial))
				If ($rowGerencial['lGerencial'] == 'Si')
				{
					...
					$lEncontrato = True ;
				       	$pdf->setY($yFila);	  
				    	$pdf->Cell($xContrato);
				    	$pdf->MultiCell($anchoPartida,$height,$rowGerencial['sWbsAnterior'],0,	'C',0);
				   	$pdf->setY($yFila);	  
				    	$pdf->Cell($xContrato + $anchoPaquete + $anchoPartida);
	...
	$pdf->Cell($anchoUnidad,$height,$rowPartidas['sMedida'],0,0,'C',0);
		...
		$pdf->setY($yFila) ;
            	...
            	If ($rowConfiguracion['sTipoPartida'] == 'Todas')
   	    	{
		    $sqlAlcances = "Select b.dCantidad, b.dAvance, b.iFase, b.sReferencia, a.sDescripcion, dCantidadAnterior, dAvanceAnterior, dCantidadActual, dAvanceActual From 
bitacoradealcances b INNER JOIN alcancesxactividad a ON (b.sContrato = a.sContrato And b.sNumeroActividad = a.sNumeroActividad And b.iFase = a.iFase) Where b.sContrato = '$sContrato' And b.dIdFecha = '$dIdFecha' And b.sIdTurno = '$sIdTurno' And b.sNumeroOrden = '$sNumeroOrden' And b.sWbs = '$rowPartidas[1]' And b.sNumeroActividad = '$rowPartidas[2]' Order By b.iFase" ;
