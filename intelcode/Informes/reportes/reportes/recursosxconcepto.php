...
function FancyTable($header,$data,$num_rows)
{
        ...
        $this->SetFillColor(100,10,150);
        $this->SetTextColor(255);
        $this->SetDrawColor(0,0,0);
        $this->SetLineWidth(.1);
        $this->SetFont('Arial','B',8);
        //Cabecera
        $w=array('30','15','15','17','15','18','18','15','15','15','32','75');
        for($i=0;$i<count($header);$i++)
                $this->Cell($w[$i],4,$header[$i],"TB",0,'C',2);
        $this->Ln();
        ...
        for($i=1;$i<=$num_rows;$i++)
        {
                ...
                $this->Cell($w[10],4,$data[$j++],"T",0,'C',$fill);
                $this->SetFont('Arial','I',7);
                $this->Multicell($w[11],4,$data[$j++],"T","L",$fill);
                $fill=!$fill;
        }
        $this->Cell(280,0,'','T',1);
}
...
while($rows=mysql_fetch_array($qry)){
        ...
        $pdf->Cell(280,7,"",0,1,'C',0);
        $pdf->Cell(40,5,"Partida",1,0,'C',1);
        $pdf->Cell(40,5,"Cantidad por Anexo",1,0,'C',1);
        $pdf->Cell(40,5,"Cantidad Reportada",1,0,'C',1);
        $pdf->Cell(40,5,"Unidad de Medida",1,1,'C',1);
        $pdf->SetTextColor(50);
        $pdf->SetFillColor(255,255,255);
        $pdf->Cell(40,5,$rows['sNumeroActividad'],1,0,'C',1);
        $pdf->Cell(40,5,$rows['dCantidadAnexo'],1,0,'C',1);
        $pdf->Cell(40,5,$rows['dInstalado'],1,0,'C',1);
        $pdf->Cell(40,5,$rows['sMedida'],1,1,'C',1);
        $pdf->SetFillColor(55,200,55);
        $pdf->Cell(280,4,"Descripcion",1,1,'C',1);
        $pdf->SetFont('Arial','',5);
        $pdf->Multicell(280,3,$rows['mDescripcion'],1,'L');
        $header=array('Insumo','F. Compra','Medida','Cant. Anexo', 'Cant. Real','Costo Anexo','Costo Real','Diferencia','Total','Total Real','Referencia','Descripcion Tecnica Detallada');
        $dato=array();
        $qry2=consulta("*","recursosanexo","sContrato='".$contrato."' and sWbs='".$rows['sWbs']."'");
        $i=-1;
        $cont=0;
        while($row=mysql_fetch_array($qry2)){
                $qry3=consulta("*","anexo_ppedido","sContrato='".$contrato."' and sIdInsumo='".$row['sIdInsumo']."' and sNumeroActividad='".$row['sNumeroActividad']."'");
                while($row3=mysql_fetch_array($qry3)){
                        if($row3['sIdInsumo']==$row['sIdInsumo']){
                                ...
                                $dato[$i++]=$row4['sMedida'];
                                $dato[$i++]=($row['dCantidad']*$rows['dCantidadAnexo']);
                                $dato[$i++]=($row3['dCantidad']*1);
                                $dato[$i++]=$row['dCostoMN'];
                                $dato[$i++]=$row3['dCosto'];
                                $dato[$i++]=($row['dCostoMN']-$row3['dCosto']);
                                $dato[$i++]=($row['dCostoMN']*$row['dCantidad']*$rows['dCantidadAnexo']);
