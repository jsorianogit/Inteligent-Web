...
if($fechaM > $fecha){
   ...
   echo "<center><h3>intente seleccionando un año y mes superior a ".nombre_mes($fecha)."</h3></center>";
   exit(0);
}
while ($fechaM <= $fecha){
...
for($n=0;$n<count($años);$n++){
	$pdf->ln(2);
	$pdf->Cell(160,9,"Fila: ".$años[$n]['fila'],1,0,$alineacion ,1);$pdf->ln();
	$pdf->Cell(160,9,"Numero de Divicion: ".$años[$n]['divFila'],1,0,$alineacion ,1);$pdf->ln();
	$pdf->Cell(160,9,"Tamaño de Division: ".$años[$n]['TamDivFila'],1,0,$alineacion ,1);$pdf->ln();
	$pdf->Cell(160,9,"Año de Division: ".$años[$n]['AñoDivFila'],1,0,$alineacion ,1);$pdf->ln();
}
*/
mysql_close();
$pdf->Output();
