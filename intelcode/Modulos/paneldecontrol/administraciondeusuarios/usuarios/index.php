...
$UsuarioIdSession = $_SESSION['sIdUsuario'];
$_SESSION['sIdUsuario'] =$UsuarioId;
...
$ocultos =array("sContrato","sPassword");
...
//update y delete
$tablasrelaciones = array ("comentariosxanexo","contratosxusuario","estimaciones","kardex_sistema","reportediario","usuariosxprograma");
//,"estimacionxproveedor"

//Array de campos llaves que relacinan a las tablas
$camposrelaciones = array ("sIdUsuario");

//consulta sql de donde tomar los campos para el formulario
//solo puede tener clausula where
$sqlGeneral="SELECT *  FROM usuarios";

//formulario objeto
$form = new usuario($sqlGeneral,$ocultos,$bloqueados,$valdefaults,$camposrelaciones,$tablasrelaciones);

//verifica las ralaciones
//$tablas = $form->relaciones("usuarios");
...
if ( isset($_REQUEST['operacion'])){
    switch($_REQUEST['operacion']){
      ...
      case ('b'):
         ...
         $form->crearDeleteUser($_SESSION['sCondicion']);
         break;
   }
}
...
if(isset($_POST['aceptar'])){
 switch ($_POST['aceptar']){
   case "Insertar":
      //hace un insert
      $form->crearInsertUser( $_POST , $HTTP_POST_FILES );
      $_POST['aceptar']=$_SESSION['sOperacion']="";
      break;
   case "Modificar":
      //hace un update
      $form->crearUpdateUser( $_SESSION['sCondicion'] , $_POST , $HTTP_POST_FILES); 
      $_POST['aceptar']=$_SESSION['sOperacion']="";
      break;
}
...
?>
<center>
<input type="button" value="<?php echo $_SESSION['sOperacion'] ?>" onClick=document.location.href='<?php echo $_SERVER[PHP_SELF] ?>?operacion=a'>
<?php
...
mysql_close();
$_SESSION['sIdUsuario'] = $UsuarioIdSession;
