...
if(isset($_POST['cIdStatus']) and 
isset($_POST['sDescripcion']) and 
$_SESSION['sOperacion']=="Insertar")
	{
		$consulta="INSERT INTO estatus VALUES ('";
		$consulta.= $_POST['cIdStatus']."','";
...
/*modifica tipo de embarcaci+on*/
if(isset($_POST['cIdStatus']) and 
isset($_POST['sDescripcion']) and 
$_SESSION['sOperacion']=="Modificar")
	{
		$CampoClave=$_SESSION['scIdStatus'];//$_POST['sTipo'];
		$consulta=" UPDATE estatus SET ";
		$consulta.= " cIdStatus='".$_POST['cIdStatus']."', ";
		$consulta.= " sDescripcion='".$_POST['sDescripcion']."' ";
		$consulta.= " WHERE cIdStatus='".$CampoClave."' ";
		mysql_query($consulta,$link);
		if (mysql_error())
		{
			echo "<script language=javaScript>alert(\"Error: ".mysql_errno()."-".mysql_error()."\")</script>";
...
if(isset($_REQUEST['reqcIdStatus']) and 
isset($_REQUEST['operacion']) and
$_REQUEST['operacion']=="b")
	{
	 	$CampoClave=trim($_REQUEST['reqcIdStatus']);
		...
		{*/
			$consulta = "DELETE FROM estatus WHERE cIdStatus='$CampoClave'";
			mysql_query($consulta,$link);
			if (mysql_error())
				echo "<script language=javaScript>alert(\"Error: ".mysql_errno()."-".mysql_error()."\")</script>";
		//}
...
if(isset($_REQUEST['order']))
	...
	$OrdenarPor="cIdStatus";

$consulta="SELECT COUNT(*) FROM estatus ORDER BY $OrdenarPor";
...
$consulta="SELECT * FROM estatus ORDER BY $OrdenarPor  LIMIT $reg1,$tampag";
$result=mysql_query($consulta,$link);
?>
<table class='enhancedtablerowhover'>
<caption>Status de Ordenes</caption>
	<thead>
	<tr>
		<td scope="col" colspan=2></td>
		<th scope="col" ><a href="?order=cIdStatus">Status</a></th>
...
while ($row = mysql_fetch_row($result)) {
	$cIdStatus=str_replace(" ","|",$row[0]);
	echo "<tr>";
	echo "<td class='CrearReporte'>
			<a href=form.php?reqcIdStatus=$cIdStatus&operacion=m>
