...
function guardar(){
	...
	mysql_query($sql);
	if(mysql_error()){$error=true; mensaje("Error: ".mysql_error()); exit();}
	...
	$sql ="UPDATE actividadesxorden SET 
			...
			WHERE sContrato='$sContrato' AND sWbs='$swbs' AND sNumeroOrden='$sNumeroOrden' AND sNumeroActividad='$sNumeroActividad' AND sIdConvenio='$convenio'";	
	if($swbs!="" and $sNumeroActividad!="")	
   	mysql_query($sql);
	if(mysql_error()){$error=true;; mensaje("Error: ".mysql_error()); exit();}
	if($error==true){
...
function actualizar(){
   ...
   //Actualizar en bitacoradeactividades
	$sql ="SELECT dCantidad,mDescripcion FROM bitacoradeactividades WHERE sContrato='$sContrato' AND dIdFecha='$fecha' AND sIdTurno='$sIdTurno' AND sNumeroOrden='$sNumeroOrden' AND sNumeroActividad='$sNumeroActividad' AND sWbs='$swbs' and sIdTipoMovimiento='$sIdTipoMovimiento'";	
	$result = mysql_query($sql);
	if($row = mysql_fetch_array($result)){
      $dCantidadAct = $row[0];
      $mDescripcion = $row[1] ." ".$comentarios ;
   }
   $cantidadTotal=$cantidadIns+$dCantidadAct;

	$sql ="UPDATE bitacoradeactividades SET dCantidad='$cantidadTotal' ,dAvance='$dAvance' , mDescripcion='$mDescripcion' WHERE sContrato='$sContrato' AND dIdFecha='$fecha' AND sIdTurno='$sIdTurno' AND sNumeroOrden='$sNumeroOrden' AND sNumeroActividad='$sNumeroActividad' AND sWbs='$swbs' and sIdTipoMovimiento='$sIdTipoMovimiento' LIMIT 1";	
	$result = mysql_query($sql);
	if(mysql_error())$error=true ;

	//Buscar coincidencias en actividadesxorden
	/*$sql = "SELECT dInstalado,dExcedente  FROM actividadesxorden  WHERE sContrato='$sContrato' AND sWbs='$swbs' AND sNumeroOrden='$sNumeroOrden' AND sNumeroActividad='$sNumeroActividad'  AND sIdConvenio='$convenio' ";
	$result=mysql_query($sql);
	if(mysql_error())$error=true ;
	if($row = mysql_fetch_array($result)){
		$dInstaladoActual = $row[0];
...
$patron=$_SESSION['patron'];#Campos no necesarios cuando son notas
#  sWbs
#  sNumeroActividad
#  dCantidad
#  dPonderado
...
//mensaje($patron . "---- ".$IdTipoMovimiento);
$nota = false;   
...
if (($fecha =="" or $sIdTurno=="" or
		$sNumeroOrden=="" or $sNumeroActividad=="" or
			$swbs=="" or $sIdTipoMovimiento=="" or $cantidad=="" or
				$cantidadIns=="" or $dPonderado=="" or $comentarios=="" ) and !$nota){
   ...
   if($volumen!="No"){
   	//Buscar coincidencias
   	$sql = "SELECT sContrato,dIdFecha,sIdTurno,sNumeroOrden,sNumeroActividad,sWbs FROM bitacoradeactividades WHERE sContrato='$sContrato' AND dIdFecha='$fecha' AND sIdTurno='$sIdTurno' AND sNumeroOrden='$sNumeroOrden' AND sNumeroActividad='$sNumeroActividad' AND sWbs='$swbs' AND sIdTipoMovimiento='$sIdTipoMovimiento'";
	  $result=mysql_query($sql);
