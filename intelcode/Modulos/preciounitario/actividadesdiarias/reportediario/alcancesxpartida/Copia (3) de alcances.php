   ...
   function Pulsar(e) {
         ...
         if (Tecla==114) {   // si se aprieta la tecla "F3"
            ...
            if(formularioVisible=="visible"){
               ...
               ventana = window.open("acumuladoAlcance.php?sWbs="+sWbs+"&sNumeroActividad="+sNumeroActividad+"&sNumeroOrden="+"<?php echo $_REQUEST['sNumeroOrden']?>","Acumulado",  "width=600,height=300,status=yes,toolbar=no,menubar=no,location=no");
                  ventana.focus();
                  }
            }
            else{
            ...
            for (j=0;j<trs.length;j++)
            {
               ...
               if(trs[j].getElementsByTagName('td').length>0)
               {
                  ...
                  trs[j].onclick=function()
                  {
                     ...
                     //alert(pos1 + " . " + this.cells[pos1].innerHTML+ " . " +  pos2 + " . " + this.cells[pos2].innerHTML);
                     Enlace = "sNumeroOrden=" + "<?php echo $sNumeroOrden?>" + "&sWbs=" + this.cells[pos1].innerHTML + "&sNumeroActividad=" +this.cells[pos2].innerHTML;                     
...
if ( isset($_REQUEST['operacion'])){
    switch($_REQUEST['operacion']){
      case ('b'):
        ...
        //echo "<br>". $sql ="  SELECT iIdDiario FROM bitacoradealcances WHERE $sNuevaCondicion and iIdDiario>=$iIdDiarioEliminar";
        $sql ="  SELECT iIdDiario FROM bitacoradealcances WHERE $sNuevaCondicion and iFase>='$iFaseEliminar' ; ";
         ...
         for($i=$iFaseEliminar ; $i<= $iUltimaFase ;$i++){
            if(count($iFases)==$i){
               ...
               $sql = "SELECT iIdDiario FROM bitacoradeactividades WHERE $sNuevaCondicion AND sIdTipoMovimiento='$sTipoOperacion'
                  AND  lAlcance='SI' ;";
               $result = mysql_query($sql);
               if($row = mysql_fetch_array($result)){
                  $Diario = $row[0];
...
<center>
<input type ="button" onClick="cargaXML('insert_destroy.php');document.location='../';" value="Regresar" title="Regresar a Reporte Diario">
<input type ="button" name='Insertar' id='Insertar' onClick="javascript:mostrarForm();" value="Insertar" title="Insertar Registro">
</center>
<br><br><br><center><table id='Vacio' border=2 bgcolor='#00AA00'><tr><td>
<?php
$result = mysql_query($sqlGeneral);
if($row = mysql_fetch_array($result)){
   ...
   echo "<font color='BLUE'><h3>Sin Registros</h3></font>";
...
<?php 
   if($_SESSION['insertar']==true){
      echo "mostrar Formulario <script>mostrarForm();</script>";
   }
   else {
      echo "ocultar Formulario <script>ocultarForm();</script>";
   }
?>
</body>
</html>
