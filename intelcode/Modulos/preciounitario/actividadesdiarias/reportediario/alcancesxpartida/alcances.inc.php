...
class alcances extends reporte {
	var $nombrecampo;
	var $editar=1;
	var $borrar=1;
 ...
 //muestra los registros de la tabla
    function visualizar($pag="",$orderby="",$nombrecampo="")
    {
        ...
        //obtener descripcion de la tablxa
        ...
        if($this->borrar==0)$conlspan--;// class='enhancedtablerowhover'
        echo "\n<center><table  name='ReporteAlcalces' id='ReporteAlcances' class='enhancedtablerowhover'>
        ...
        while ($row = mysql_fetch_array($result))
        {
	     ...
	     $filtro = $this->cifrar($row, $result);
            ...
            echo "\n<td class='CrearReporte'>
				   \n<a title='Editar Comentarios' onClick=\"window.open('comentarios.php?$filtro','Comentarios','width=600,height=500,scrollbars=yes,resizable=yes,status=0,toolbar=0');\" href='#'> 
    ...
    function actiOrdenes($condicion){
    	...
    	$sql = "show fields from alcancesxactividad";
	...
	//	$sql = "SELECT dInstalado FROM alcancesxactividad WHERE $con\n";
	//	$result = mysql_query($sql);
     //   if($row = mysql_fetch_array($result)){
	//		$dCantidadac = $row[0] ;
	//	}
	...
	//	$NuevodInstalado;
	//	$sqlorden = "UPDATE alcancesxactividad SET dInstalado=$NuevodInstalado WHERE $con\n";
		//return;
        //eliminar el actual
