...
#Calcular Avances Fisicos
function  avanceFisico ($sContrato,$sNumeroOrden, $sIdConvenio, $sIdTurno, $dIdFecha) 
{

    $dAvancePlataforma = 0 ;
    $dAvanceObra       = 0 ;
    $dAvancePartida    = 0 ;
	$sql="SELECT lStatus, sIdConvenio From reportediario 
    ...
    {
        $dAvanceObra = 0 ;
		 $_sql="Select Sum(( b.dAvance * a.dPonderado) / 100 ) as dAvance From bitacoradeactividades b 
                           INNER JOIN actividadesxorden a 
									ON (b.sContrato = a.sContrato And b.sNumeroOrden = a.sNumeroOrden And a.sIdConvenio = '$sIdConvenio' 
		  ...
		  if($_row = mysql_fetch_array($_result)){
				$dAvanceObra = $dAvancePlataforma + $_row['dAvance'] ;
		  }
		  $_sql = "Select sum(dAvance) as dAvance from avancesglobalesxorden 
		  			 WHERE sContrato = '$sContrato' and sIdConvenio = '$sIdConvenio' 
							 And dIdFecha < '$dIdFecha' and sNumeroOrden = '$sNumeroOrden' 
					 GROUP BY sContrato" ;
        $_result = mysql_query($_sql);
        if($_row = mysql_fetch_array($_result)){
		  		If (($_row['dAvance'] + $dAvanceObra ) > 100 )
                  $dAvanceObra = 100 - $_row['dAvance']  ;
			
		  }

  		 $_sql = "INSERT INTO avancesglobalesxorden (sContrato, sNumeroOrden, dIdFecha, sIdConvenio, sIdTurno, dAvance)
							 Values ('$sContrato', '', '$dIdFecha', '$dIdFecha', 'A', '$dAvanceObra')
							 ON DUPLICATE KEY UPDATE dAvance='$dAvanceObra'";
       mysql_query($_sql);
    }
   
	$dAvanceObra = round($dAvanceObra*10000)/10000;
	 return $dAvanceObra ;
   
     
}
#Calcular Avances Fisicos De todo el Contrato
function  avanceFisicoContrato ($sContrato, $sIdConvenio, $sIdTurno, $dIdFecha) 
{
    $dAvancePlataforma = 0 ;
    $dAvanceObra       = 0 ;
    $dAvancePartida    = 0 ;
