...
class PDF extends FPDF
{
   ...
   function PrintImage()
   {
      ...
      while($NumImg<$Tamanio)
      {
         ...
         $this->MultiCell(90,3,$nameFile[$NumImg][4]/*$nameFile[$NumImg][2]*/,0,'C');
         ...
         $this->setY($y+9);
         $this->MultiCell(50,2,/*$nameFile[$NumImg][2]*/"",0,'C');
         $this->setY($y+9);
         $this->cell(150);
   ...
   function Footer()
   {
     ...
     if($sIdTurno =="")$sIdTurno =  $_SESSION['ssIdTurno'];

     if(strpos($sNumeroOrden,"CONTRATO")===false)
      $buscarOrden = " and sNumeroOrden='$sNumeroOrden'";
     else
      $buscarOrden = " ";

     $sql = " select lStatus,sIdUsuario,sIdUsuarioAutoriza,sIdUsuarioValida,sIdUsuarioResidente
              from reportediario
              where sContrato='$sContrato' and dIdFecha ='$dFecha' $buscarOrden
     ...
     //Consultas
     $sql="SELECT * FROM firmas WHERE sContrato='$sContrato' $buscarOrden AND dIdFecha = '$dFecha'";
     $result=mysql_query($sql,$link);
     if ($row=mysql_fetch_array($result))
     {
   ...
   {
      $sql="SELECT * FROM firmas WHERE sContrato='$sContrato' $buscarOrden AND dIdFecha<='$dFecha' order by dIdFecha DESC;";
      $result=mysql_query($sql,$link);
      if ($row=mysql_fetch_array($result))
      {
         $sFirmante1=$row['sFirmante1'];
