...
//Obtenes las jornadas x dia, regresa un array con las jornadas diarias de nombre $iJornadas
$cancelar = "no";
...
if ($row = mysql_fetch_array($result)){
	if($row[0] != $_SESSION['sNumeroOrden'] AND $_POST['sTipo']=="Disponibilidad del Sitio"){
		?>
		<script language="javascript">
			var valor;
			valor = 0 ;
			valor = confirm ("Esta Ingresando una Disponibilidad con una Plataforma Destino Diferente a la Predeterminada (<?php echo $row[1] ?>), desea Continuar?") ? "siAceptar" : "siCancelar";
			if(valor=="siCancelar"){
				document.location="menu.php";
			}
			else{
				document.write("<?php require(insert.php); ?>");
			}
		//	if(valor=="siCancelar")
		//			window.stop();
		</script>
