...
?>
<form name="numerodeorden" method=post action="index.php">
	Seleccion Numero de Orden<select name="orden" onChange="document.numerodeorden.submit();">
	<?php
		$consulta="SELECT sNumeroOrden FROM ordenesdetrabajo WHERE sContrato='$sContrato' ORDER BY sNumeroOrden";
		$result=mysql_query($consulta,$link);	
		echo "<option>Seleccionar</option>";
		while($row=mysql_fetch_array($result)){
		 	if (isset($_POST['orden']) and $_POST['orden']==$row[0])$Seleccionar="selected";
		 	else $Seleccionar="";
			echo "<option $Seleccionar>$row[0]</option>";
		}
	?>
	</select>	
</form>
...
if (isset($_POST['orden']))
{
  if(file_exists("Grafica.png"))
      unlink("Grafica.png");
 	$sNumeroOrden=$_POST['orden'];
	$consulta="SELECT dIdFecha,dAvancePonderadoDia,dAvancePonderadoGlobal FROM avancesglobales WHERE 
sContrato='$sContrato' AND sNumeroOrden='$sNumeroOrden' AND sIdConvenio ='$sIdConvenio' ORDER BY dIdFecha";
	$result=mysql_query($consulta,$link);
	?>
	<!--	<table class='enhancedtablerowhover'>
	<caption>Avance Progamado/Fisico</caption>
		<thead>
		<tr>
...
if(!isset($_SESSION['ssContrato']))
	exit();
//Obtener datos
$sql="SELECT max(dIdFecha) as dIdFecha FROM reportediario WHERE sContrato = '$sContrato' AND sNumeroOrden = '$sNumeroOrden' ";
$result=mysql_query($sql,$link);
if($row=mysql_fetch_array($result))
	$FecUltimoReporte =  $row['dIdFecha'];

$ultimo_avance = "select max(dIdFecha) as dIdFecha from avancesglobales Where sContrato = '$sContrato' and sNumeroOrden = '$sNumeroOrden' and sIdConvenio = '$sIdConvenio'" ;
$result=mysql_query($ultimo_avance ,$link);
if($row=mysql_fetch_array($result))
	$FecUltimoAvance =  $row['dIdFecha'];

$inicio_grafica = "select min(dIdFecha) as dIdFecha from avancesglobales Where sContrato = '$sContrato' and sNumeroOrden = '$sNumeroOrden' and sIdConvenio = '$sIdConvenio'" ;
...
{
	$qryProgramado = "select max(dAvancePonderadoGlobal) as dAvancePonderadoGlobal from avancesglobales Where sContrato = '$sContrato' and sNumeroOrden = '$sNumeroOrden' and dIdFecha <= '$Fechas[$cont]' and sIdConvenio ='$sIdConvenio' order by dIdFecha DESC";
...
for($cont=0;$cont<=$i-1;$cont++)
{
	$qryProgramado = "select sum(dAvance) as dAvance from avancesglobalesxorden Where sContrato = '$sContrato' and sNumeroOrden = '$sNumeroOrden' and dIdFecha <= '$Fechas[$cont]' and sIdConvenio ='$sIdConvenio' group by sNumeroOrden";
	$result=mysql_query($qryProgramado,$link);
	if ($row=mysql_fetch_array($result)){
			$ultimo_fisico = $row[0];
	}
...
$Sql="SELECT sum(a.dCantidad * b.dVentaMN) FROM actividadesxorden a INNER JOIN actividadesxanexo b ON (a.sContrato=b.sContrato AND a.sIdConvenio=b.sIdConvenio AND a.sNumeroActividad=b.sNumeroActividad AND a.sTipoActividad='Actividad') WHERE a.sContrato = '$sContrato' and a.sNumeroOrden = '$sNumeroOrden' and a.sIdConvenio ='$sIdConvenio'  GROUP BY a.sNumeroOrden";
$result=mysql_query($Sql,$link);
if($row=mysql_fetch_array($result))
{
	 $MontoOrden=$row[0];
...
//informacion del contrato y empresa
