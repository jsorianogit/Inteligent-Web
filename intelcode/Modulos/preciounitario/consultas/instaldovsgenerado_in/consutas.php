...
<tr>
	...
	<td>
		...
		<input type="checkbox" checked value="si" <?php echo $consolidar=="si" ? "checked":""; ?> name="consolidar" >
	</td>
</tr>
...
$sql="Select dCantidadAnexo, sMedida, dVentaMN, dPonderado, dInstalado, dExcedente , (dCantidadAnexo-dInstalado) from 
actividadesxanexo Where sContrato = '$sContrato' and sIdConvenio = '$sIdConvenio' and sNumeroActividad='$actividad'";
$sqlDescripcion="Select mDescripcion from              
actividadesxanexo Where sContrato = '$sContrato' and sIdConvenio = '$sIdConvenio' and sNumeroActividad='$actividad'";
$titulo = array("Cantidad","Medida","Precio MN","Ponderado",
...
$resultado->ponerconsulta($sqlDescripcion,1,$titulo);
$resultado->imprimir();
echo "</td></tr>";

//imprime las actividades segun la fecha
$sql="Select sNumeroOrden, sPaquete, sNumeroActividad, substr(mDescripcion,1,25) as mDescripcion, dCantidad,  dFechaInicio, 
dFechaFinal, dInstalado, dExcedente,(dCantidad-dInstalado) as dPendiente, dPonderado
...
$titulo = array("Orden","Paquete","Actividad","Descripcion","Cantidad","Fecha Inicio","Fecha Final","Instalado","Excedente","Pendiente","Ponderado");
$result = mysql_query($sql);
echo "<tr><td colspan='2' valign='top' align='center'>";
echo "<table class='enhancedtablerowhover'>\n";
echo "<tr><td colspan = 11><center>Reportes y Generadores Encontrados en las Diferentes Ordenes</center></td><tr>";
echo "<th>Orden</th>\n";
echo "<th>Paquete</th>\n";
...
while($row = mysql_fetch_array($result)){
	...
	echo "<td><a href='coincidencias.php?orden=".$row['sNumeroOrden']."&wbs=".$row['sPaquete']."&actividad=".$actividad."&fecha=".$Fecha."&consolidar=$consolidar' target='coincidencias'>".$row['sPaquete']."</a></td>\n";
