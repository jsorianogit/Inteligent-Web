...
$sql = "select   b.sContrato, b.dIdFecha, b.iIdDiario, b.sIdTurno, b.sNumeroOrden,   b.sWbs, b.sNumeroActividad, b.dCantidad, b.dAvance, b.sIdTipoMovimiento, b.lAlcance, r.sNumeroReporte, r.sIdConvenio, tu.sOrigenTierra, tu.sDescripcion as sDescripcionTurno,r.lStatus from   bitacoradeactividades b INNER JOIN reportediario r ON (r.sContrato = b.sContrato And r.sNumeroOrden = b.sNumeroOrden And r.dIdFecha = b.dIdFecha And r.sIdTurno = b.sIdTurno) INNER JOIN  turnos tu ON (r.sContrato = tu.sContrato And r.sIdTurno = tu.sIdTurno) Where  b.sContrato = '$sContrato' and b.sNumeroOrden =  '$sNumeroOrden' and b.sWbs = '$sWbs' and b.sNumeroActividad= '$sNumeroActividad ' Order By   b.dIdFecha, b.iIdDiario asc";
...
while ($row = mysql_fetch_array($result)){
   ...
   print ("<tr>");
   print ("<td><a href='../../../Acceso/scripts/php/Reportes/generadores/rptReporteDiario?sContrato=".$row['sContrato']."&sNumeroOrden=".$row['sNumeroOrden']."&dFecha=".$row['dIdFecha']."&sIdTurno=".$row['sIdTurno']."&sIdConvenio=".$row['sIdConvenio']."&lStatus=".$row['lStatus']."' target=_blank>");
