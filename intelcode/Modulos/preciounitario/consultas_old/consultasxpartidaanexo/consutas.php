...
<td>
<?php
$sql = " select sContrato,sIdConvenio,sNumeroActividad,dPonderado,sMedida,sWbs,sWbsAnterior
...
<?php
//
$sql="Select sNumeroOrden, sWbsAnterior, sWbs, sNumeroActividad, mDescripcion,sMedida, dCantidad, dVentaMN, dFechaInicio, dFechaFinal, dInstalado, dExcedente, dPonderado From actividadesxorden Where sContrato = '$sContrato' And sIdConvenio ='$sIdConvenio' And sNumeroActividad = '$actividad' and sTipoActividad = 'Actividad' order by sNumeroOrden, iItemOrden";
$result = mysql_query($sql);
...
while ($row = mysql_fetch_array($result)){
   ...
   print ("<td><a href = 'coincidencias.php?sNumeroOrden=$row[sNumeroOrden]&sWbs=$row[sWbs]&sNumeroActividad=$row[sNumeroActividad]&sContrato=$sContrato' target='coincidencias'>".$row['sWbs']."</td></a>");
