...
//Class definition
class frmTiposdeEmbarcacion extends Page
{
   public $txtTitulo = null;
   public $txtRenglon = null;
   ...
   function frmTiposdeEmbarcacionBeforeShow($sender, $params)
   {
      global $Connection;
      $Connection->conectar();
      //      global $sContrato;
      ...
      $sql = "select * from tiposdeembarcacion";
   ...
   function cmdAceptarClick($sender, $params)
   {
      ...
      try
      {
         $Id = $this->txtIdEmbarcacion->Text;
         $Descripcion = $this->txtDescripcion->Text;
         $Renglon = $this->txtRenglon->Text;
         ...
         if($Opcion == "Modificar")
         {
            $sql = "update tiposdeembarcacion
                                set
                                   sIdTipoEmbarcacion = '$Id',
                                   sDescripcion       = '$Descripcion',
                                   iRenglon           = '$Renglon',
                                   sTitulo            = '$Titulo'
                                where
                                   sIdTipoEmbarcacion = '$IdOld'";
         }
   ...
   function cmdEliminarJSClick($sender, $params)
   {
      ?>               //Add your javascript code here
                    ...
                    ok  = 0;
                    if (i>0)
                    {   if (!confirm("  Desea ELIMINAR El Tipo de Embarcacion Seleccionada ?"))
                           ok = 1;
                    }
                    if (ok == 1)
                       return false;
   ...
   function cmdEliminarClick($sender, $params)
   {
      ...
      try
      {
         $sql = "delete from tiposdeembarcacion
                           where  sIdTipoEmbarcacion = '$IdOld'";
         $Connection->Query4->close();
         $Connection->Query4->setSQL($sql);
         $Connection->Query4->open();
         $Connection->Query4->close();
   ...
   function cmdAgregarJSClick($sender, $params)
   {
      ?>              //Add your javascript code here
                 ...
                 document.getElementById("txtRenglon").value       = "";
   ...
   function dumpJavascript()
   {
      echo 'function controlBotones( habilitar ){
                        ...
                        document.getElementById("txtRenglon").disabled = accion;
                        document.getElementById("txtTitulo").disabled = accion;
      ...
      echo 'function ResaltarBotones()
                      {
                          ...
                          document.getElementById("txtRenglon").style.backgroundColor = "";
                          document.getElementById("txtTitulo").style.backgroundColor = "";
                            ...
                            (  function(index)
                               {
                                  ...
                                  sReglon        = Tabla.getValue(3, index);
                                  sTitulo        = Tabla.getValue(4, index);
                                  document.getElementById("txtIdEmbarcacion").value = sId;
                                  document.getElementById("txtDescripcion").value   = sDescripcion;
                                  document.getElementById("txtRenglon").value       = sReglon;
                                  document.getElementById("txtTitulo").value        = sTitulo;
   ...
   function txtRenglonJSKeyPress($sender, $params)
   {
   ...
   function txtRenglonJSFocus($sender, $params)
   {
   ...
   function txtRenglonJSBlur($sender, $params)
   {
   ...
   function txtTituloJSKeyPress($sender, $params)
   {
   ...
   function txtTituloJSFocus($sender, $params)
   {
   ...
   function txtTituloJSBlur($sender, $params)
   {
...
global $frmTiposdeEmbarcacion;

//Creates the form
$frmTiposdeEmbarcacion = new frmTiposdeEmbarcacion($application);

//Read from resource file
$frmTiposdeEmbarcacion->loadResource(__FILE__);

//Shows the form
$frmTiposdeEmbarcacion->show();
