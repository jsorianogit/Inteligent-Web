...
//var pathAbsolute="http://127.0.0.1/intelcode/";
var path = pathAbsolute;//+'intelcode/Modulos/include/acc_calendar/';
//alert(pathAbsolute);
var preposiciones = {
...
var title_enlace = {
   es : 'Desplegar para seleccionar la fecha para1',
...
function cargaLinks() {
   for (var i = 0; i < labels.length; i++) {
      ...
      if(elasociado.type == 'text' && elasociado.className.indexOf(label_class[lang.substr(0,2)]) != -1) {
         ...
         enl_cal_img.setAttribute('src',path + 'acc_calendar.png'); //'acc_calendar/acc_calendar.png'
         ...
         if(elasociado.className.indexOf('todnact') != -1)
            ...
            todact = true;
         enl_cal.setAttribute('href','javascript:calendario(' + todact + ',' + Hoy_real.getMonth() + ',"","","' + dest +'",' + rango +')');
...
function calendario(todact,mes,year,nuevo,id,rango,dia) {
   ...
   for(var i = 0; i < celda.length; i++){
      ...
      enl_dia.setAttribute('href','javascript:send_date(' + todact + ',"' + calendario_tbody_tr_td_text.nodeValue + '/' + (Hoy.getMonth()+1) + '/' + (Hoy.getYear()+comp) + '","'+ id +'",'+ rango +')');
   ...
   if(!document.getElementById('calendario')){
      ...
      zindex(calendar);
      contador = contador + 1;
      calendar.style.top = 10 + 'px' ; //(document.getElementById(id).offsetHeight + document.getElementById(id).offsetTop) + 'px';
      calendar.style.left =10 + 'px' ; // (document.getElementById('cal_' + id).offsetLeft-2*(calendar.offsetWidth/3)) + 'px';
   }
   ...
   if(!document.getElementById('niquelao')) {
      ...
      niquelao.src = path + 'niquelao.png'; //'acc_calendar/acc_calendar.png'
      niquelao.setAttribute('alt','intelcode.dyndns.org');
